# data-analysis-
### Task 2: Tableau Dashboard
- Created interactive dashboard using Superstore dataset  
- Visualized sales, profit, and trends  
- (https://public.tableau.com/app/profile/d.siva.kumar/viz/Book1_17545743996530/Dashboard1)

 ### Task 3: Tableau Dashboard

- Created an interactive dashboard using Tableau Public  
- Visualized data insights using charts and filters  
- Enabled better understanding of trends and patterns  

🔗 Dashboard Link:
https://public.tableau.com/app/profile/d.siva.kumar/viz/Book1_17545743996530/Dashboard1?publish=yes

### Task 8: Advanced Tableau Dashboard

- Developed an interactive dashboard using Tableau Public  
- Visualized key insights using multiple charts and filters  
- Improved data understanding through interactive visualizations  

🔗 Dashboard Link:
https://public.tableau.com/app/profile/d.siva.kumar/viz/Book1_17545743996530/Dashboard1?publish=yes
